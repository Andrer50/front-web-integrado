...
export function AppointmentList({ patientId }: AppointmentListProps) {
  ...
  // 1. Obtener Citas de Próximas Citas (CONFIRMED o PENDING)
  ...
  // 2. Obtener Citas de Historial (COMPLETED o CANCELLED)
  const { data: historyAppointmentsData, isLoading: isLoadingHistory } =
    useAppointments({
      patientId: patientId,
      status: "COMPLETED", // o CANCELLED, por simplicidad filtramos COMPLETED en historial
      page: historyPage,
      size: 5,
    });
  ...
  // Filtrado simple en el lado del cliente por query de búsqueda
